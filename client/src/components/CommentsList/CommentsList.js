  ...
  const fetchComments = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:4000/api/comment/comments/${postId}`,
        {
          headers: {
  ...
  return (
    <div>
      <form  onSubmit={handleCommentSubmit}>
        ...
        />
        <button style={{color:"white"}} type="submit">Add Comment</button>
      </form>
      ...
      {comments.length > 0 ? (
        ...
        <p className="pppp">No comments yet. Be the first to comment!</p>
      )}
    </div>
  );
};
